...
class OrdenForm(forms.ModelForm):
    ...
    cliente_existente = forms.ModelChoiceField(
        queryset=Cliente.objects.all(), required=False, label="Cliente Registrado"
    )
    # Campos para registrar un nuevo cliente (en caso de que no exista)
    nuevo_cliente_nombre = forms.CharField(
        max_length=100, required=False, label="Nuevo Cliente (Nombre)"
    )
    nuevo_cliente_telefono = forms.CharField(
        max_length=20, required=False, label="Nuevo Cliente (Teléfono)"
    )
    nuevo_cliente_ruta = forms.CharField(
        max_length=50, required=False, label="Nuevo Cliente (Ruta)"
    )

    # Campo para seleccionar un motor ya registrado
    motor_existente = forms.ModelChoiceField(
        queryset=Motor.objects.all(), required=False, label="Motor Registrado"
    )
    # Campos para registrar un nuevo motor (en caso de que no exista)
    nuevo_motor_nombre = forms.CharField(
        max_length=100, required=False, label="Nuevo Motor (Nombre)"
    )
    nuevo_motor_especificaciones = forms.CharField(
        widget=forms.Textarea, required=False, label="Nuevo Motor (Especificaciones)"
    )
    imagen = forms.ImageField(required=False, label="Imagen Adjunta")
    ...
    def __init__(self, *args, **kwargs):
        ...
        for item in items:
            ...
            self.fields[checkbox_name] = forms.BooleanField(
                required=False, label=item.nombre
            )
            self.fields[quantity_name] = forms.IntegerField(
                required=False, min_value=1, initial=1, label="Cantidad"
            )
            ...
            self.item_list.append(
                (checkbox_name, quantity_name, comment_name, item.nombre)
            )
        ...
        self.fields[custom_checkbox] = forms.BooleanField(
            required=False, label="Otro ítem (no registrado)"
        )
        self.fields[custom_name] = forms.CharField(
            required=False, label="Nombre del ítem"
        )
        self.fields[custom_quantity] = forms.IntegerField(
            required=False, min_value=1, initial=1, label="Cantidad"
        )
        self.fields[custom_comment] = forms.CharField(
            required=False,
            widget=forms.Textarea(attrs={"class": "form-control", "rows": 2}),
            label="Comentario",
        )
        self.item_list.append(
            (custom_checkbox, custom_name, custom_quantity, custom_comment, "Otro ítem")
        )
...
class ServicioSelectionForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        for servicio in CatalogoServicio.objects.all():
            ...
            self.fields[selected_field] = forms.BooleanField(
                required=False, label=servicio.nombre
            )
            self.fields[quantity_required_field] = forms.BooleanField(
                required=False, initial=False
            )
            self.fields[quantity_field] = forms.CharField(
                required=False, initial="", label="Cantidad"
            )
            self.serv_list.append(
                (
                    selected_field,
        ...
        self.fields[custom_selected] = forms.BooleanField(
            required=False, label="Otro servicio (no registrado)"
        )
        self.fields[custom_name] = forms.CharField(
            required=False, label="Nombre del servicio"
        )
        self.fields[custom_quantity_required] = forms.BooleanField(
            required=False, initial=False
        )
        self.fields[custom_quantity] = forms.CharField(
            required=False, initial="", label="Cantidad"
        )
        self.serv_list.append(
            (
